...
public class CatalogoController {
    ...
    @GetMapping public String list(@RequestParam(required=false) String category, @RequestParam(required=false) String q, @RequestParam(required=false) String listinoType, @RequestParam(required=false) String added, Model model) throws Exception { Set<String> selectedCodes = cart.items().stream().map(i -> i.getProduct_code()).collect(Collectors.toSet()); model.addAttribute("products", products.findAll(category,q,listinoType)); model.addAttribute("categories", products.categories()); model.addAttribute("selectedCategory", category); model.addAttribute("selectedListinoType", listinoType); model.addAttribute("q", q); model.addAttribute("addedCode", added); model.addAttribute("selectedCodes", selectedCodes); model.addAttribute("cartCount", cart.items().size()); return "catalogo"; }
    @PostMapping("/add/{code}") public String add(@PathVariable String code, @RequestParam(required=false) String category, @RequestParam(required=false) String q, @RequestParam(required=false) String listinoType) throws Exception {
        cart.add(code);
